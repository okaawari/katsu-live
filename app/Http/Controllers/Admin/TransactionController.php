...
class TransactionController extends Controller
{
    ...
    {
        $query = PaymentHistory::whereBetween('transaction_date', [$startDate, $endDate]);
        ...
        $prevQuery = PaymentHistory::whereBetween('transaction_date', [$prevStartDate, $prevEndDate]);
        if ($userId) {
        ...
        switch ($period) {
            ...
            case 'week':
                return $query->select(
                    ...
                    DB::raw('SUM(amount) as revenue'),
                    DB::raw('COUNT(*) as transactions'),
                    DB::raw('MIN(DATE(transaction_date)) as start_date'),
                    DB::raw('MAX(DATE(transaction_date)) as end_date')
                )
                ->groupBy('period')
                ->orderBy('period')
                ->get();
    ...
    private function getMonthlyGrowthData()
    {
        ...
        return PaymentHistory::where('transaction_date', '>=', $startDate)
            ->select(
