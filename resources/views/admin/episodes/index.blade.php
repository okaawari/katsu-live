...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
        <form method="GET" action="{{ route('admin.episodes.index') }}" class="space-y-4 sm:space-y-0 sm:flex sm:items-center sm:space-x-4">
            <div class="flex-1">
                    ...
                    @forelse($episodes as $episode)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                ...
                                <div class="flex items-center">
                                    <div class="w-12 h-12 flex-shrink-0">
                                        @if($episode->poster_image)
                                            <img class="h-12 w-12 rounded-lg object-cover" src="{{ asset('storage/' . $episode->poster_image) }}" alt="Episode {{ $episode->episode_number }}">
                                        @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div class="text-sm text-gray-500 dark:text-gray-400">
                                    {{ $episode->anime->category->name ?? 'No Category' }}
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2">
                                    ...
                                    <form method="POST" action="{{ route('admin.episodes.destroy', $episode) }}" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" onclick="return confirm('Are you sure you want to delete this episode? This will also delete all associated files.')" class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300">
                                            Delete
                                        </button>
                                    </form>
                                </div>
