...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <h2 class="text-xl font-semibold text-gray-900 dark:text-white">
            Edit Episode {{ $episode->episode_number }} - {{ $episode->anime->title }}
        </h2>
        <div class="flex space-x-3">
            <a href="{{ route('admin.episodes.show', $episode) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 text-sm font-medium rounded-md text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg">
        <form method="POST" action="{{ route('admin.episodes.update', $episode) }}" enctype="multipart/form-data" class="space-y-6 p-6" id="episode-form">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="episode_number" id="episode_number" value="{{ old('episode_number', $episode->episode_number) }}" required min="1"
                           class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-2 px-3 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-green-500 focus:border-green-500">
            ...
            <div>
                ...
                <textarea name="synopsis" id="synopsis" rows="4"
                          class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-2 px-3 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-green-500 focus:border-green-500">{{ old('synopsis', $episode->synopsis) }}</textarea>
                @error('synopsis')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
                <div>
                    ...
                    <select name="status" id="status" required
                        ...
                        <option value="scheduled" {{ old('status', $episode->status) === 'scheduled' ? 'selected' : '' }}>Scheduled</option>
                        <option value="published" {{ old('status', $episode->status) === 'published' ? 'selected' : '' }}>Published</option>
                        <option value="archived" {{ old('status', $episode->status) === 'archived' ? 'selected' : '' }}>Archived</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="visibility" id="visibility" required
                            class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-2 px-3 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-green-500 focus:border-green-500">
                        <option value="public" {{ old('visibility', $episode->visibility) === 'public' ? 'selected' : '' }}>Public</option>
                        <option value="private" {{ old('visibility', $episode->visibility) === 'private' ? 'selected' : '' }}>Private</option>
                        <option value="premium" {{ old('visibility', $episode->visibility) === 'premium' ? 'selected' : '' }}>Premium</option>
                    </select>
                    @error('visibility')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="datetime-local" name="scheduled_at" id="scheduled_at" 
                           value="{{ old('scheduled_at', $episode->scheduled_at ? $episode->scheduled_at->format('Y-m-d\TH:i') : '') }}"
                           class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-2 px-3 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-green-500 focus:border-green-500">
                    @error('scheduled_at')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex items-center space-x-4 pt-6">
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $episode->is_featured) ? 'checked' : '' }}
                    ...
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="is_premium" value="1" {{ old('is_premium', $episode->is_premium) ? 'checked' : '' }}
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-1 text-center">
                            ...
                            <div class="flex text-sm text-gray-600 dark:text-gray-400">
                                <label for="poster_image" class="relative cursor-pointer bg-white dark:bg-gray-700 rounded-md font-medium text-green-600 hover:text-green-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-green-500">
                                    <span>{{ $episode->poster_image ? 'Replace poster image' : 'Upload poster image' }}</span>
                ...
                <div>
                    ...
                    @if($episode->thumbnail_image)
                        <div class="mt-2 mb-4">
                            <p class="text-sm text-gray-500 dark:text-gray-400 mb-2">Current Thumbnail:</p>
                            <img class="h-32 w-32 object-cover rounded-lg" src="{{ asset('storage/' . $episode->thumbnail_image) }}" alt="Current thumbnail">
                        </div>
                    @endif
                        ...
                        <div class="space-y-1 text-center">
                            ...
                            <div class="flex text-sm text-gray-600 dark:text-gray-400">
                                <label for="thumbnail_image" class="relative cursor-pointer bg-white dark:bg-gray-700 rounded-md font-medium text-green-600 hover:text-green-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-green-500">
                                    <span>{{ $episode->thumbnail_image ? 'Replace thumbnail' : 'Upload thumbnail' }}</span>
            ...
            <div>
                ...
                @if($episode->video_720p)
                    <div class="mt-2 mb-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                        <p class="text-sm text-gray-500 dark:text-gray-400 mb-2">Current Video:</p>
                        <div class="flex items-center space-x-2">
                            <svg class="w-5 h-5 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                            </svg>
                            <span class="text-sm text-gray-700 dark:text-gray-300">{{ basename($episode->video_720p) }}</span>
                        </div>
                    </div>
                @endif
                    ...
                    <div class="space-y-1 text-center w-full">
                        ...
                        <div class="flex text-sm text-gray-600 dark:text-gray-400">
                            <label for="video_720p" class="relative cursor-pointer bg-white dark:bg-gray-700 rounded-md font-medium text-green-600 hover:text-green-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-green-500">
                                <span>{{ $episode->video_720p ? 'Replace video file' : 'Upload video file' }}</span>
                                <input id="video_720p" name="video_720p" type="file" class="sr-only" accept="video/*" onchange="handleVideoUpload(this)">
                            </label>
                            <p class="pl-1">or drag and drop</p>
                        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                    ...
                    @if($episode->subtitle_mongolian)
                        <div class="mt-2 mb-4 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                            <p class="text-sm text-gray-500 dark:text-gray-400 mb-1">Current Mongolian Subtitles:</p>
                            <span class="text-sm text-gray-700 dark:text-gray-300">{{ basename($episode->subtitle_mongolian) }}</span>
                        </div>
                    @endif
                        ...
                        <div class="space-y-1 text-center">
                            ...
                            <div class="flex text-sm text-gray-600 dark:text-gray-400">
                                <label for="subtitle_mongolian" class="relative cursor-pointer bg-white dark:bg-gray-700 rounded-md font-medium text-green-600 hover:text-green-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-green-500">
                                    <span>{{ $episode->subtitle_mongolian ? 'Replace Mongolian subtitles' : 'Upload Mongolian subtitles' }}</span>
                ...
                <div>
                    ...
                    @if($episode->subtitle_english)
                        <div class="mt-2 mb-4 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                            <p class="text-sm text-gray-500 dark:text-gray-400 mb-1">Current English Subtitles:</p>
                            <span class="text-sm text-gray-700 dark:text-gray-300">{{ basename($episode->subtitle_english) }}</span>
                        </div>
                    @endif
                        ...
                        <div class="space-y-1 text-center">
                            ...
                            <div class="flex text-sm text-gray-600 dark:text-gray-400">
                                <label for="subtitle_english" class="relative cursor-pointer bg-white dark:bg-gray-700 rounded-md font-medium text-green-600 hover:text-green-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-green-500">
                                    <span>{{ $episode->subtitle_english ? 'Replace English subtitles' : 'Upload English subtitles' }}</span>
    ...
    if (input.files && input.files[0]) {
        ...
        const interval = setInterval(() => {
            ...
            if (progress >= 100) {
                ...
                submitBtn.innerHTML = 'Update Episode';
