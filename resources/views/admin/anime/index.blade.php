...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-4 sm:space-y-0">
        ...
        <a href="{{ route('admin.anime.create') }}" class="inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-4 lg:p-6">
        <form method="GET" action="{{ route('admin.anime.index') }}" class="space-y-4">
            ...
            <div class="flex flex-col sm:flex-row gap-2">
                <button type="submit" class="inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                    Search
                </button>
    ...
    <!-- Anime List - Mobile Cards / Desktop Table -->
                ...
                <div class="p-4 border-b border-gray-200 dark:border-gray-700 last:border-b-0">
                    <div class="flex items-start space-x-4">
                        <div class="flex-shrink-0">
                            @if($item->cover_image)
                                <img class="h-16 w-16 rounded-lg object-cover" src="{{ asset('storage/' . $item->cover_image) }}" alt="{{ $item->title }}">
                            @else
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex flex-col items-end space-y-1">
                                    ...
                                    <span class="text-xs text-gray-500 dark:text-gray-400">
                                        {{ $item->episodes()->count() }} / {{ $item->total_episodes ?? '∞' }} eps
                                    </span>
                                </div>
                            </div>
                            <div class="mt-2 flex items-center justify-between">
                                ...
                                <div class="flex space-x-2">
                                    <a href="{{ route('admin.anime.show', $item) }}" class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300 text-sm">
                                        View
                                    </a>
                                    <a href="{{ route('admin.anime.edit', $item) }}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300 text-sm">
                                        Edit
                                    </a>
                                    <form method="POST" action="{{ route('admin.anime.destroy', $item) }}" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" onclick="return confirm('Are you sure you want to delete this anime?')" class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300 text-sm">
                                            Delete
                                        </button>
                                    </form>
                                </div>
                    ...
                    @forelse($anime as $item)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                                {{ $item->episodes()->count() }} / {{ $item->total_episodes ?? '∞' }}
                            </td>
